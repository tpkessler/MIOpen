...
struct ThreadwiseGenericTensorSliceCopy_v5
{
    ...
    static constexpr auto GetRegBuffer();

    template <>
    static constexpr auto GetRegBuffer<float, 8>()
    {
        return float_vec8_t{};
    }

    float_vec8_t ThreadBuffer;
    ...
    struct vector_data_load<float, 4, 4>
    {
        ...
        __device__ static float_vec4_t run(const float* p_src, const SrcCoord src_coord_begin)
        {
            ...
            r.s4 = load_data<float4_t, float>(p_src, src_coord.GetOffset());
    ...
    struct vector_data_load<float, 1, 4>
    {
        ...
        __device__ static float_vec4_t run(const float* p_src, const SrcCoord src_coord_begin)
        {
            ...
            scalar_id(vector_access_dim) = 0;
            auto src_coord               = src_coord_begin + scalar_id;
            r.s1(Number<0>{})            = load_data<float, float>(p_src, src_coord.GetOffset());

            scalar_id(vector_access_dim) = 1;
            src_coord                    = src_coord_begin + scalar_id;
            r.s1(Number<1>{})            = load_data<float, float>(p_src, src_coord.GetOffset());

            scalar_id(vector_access_dim) = 2;
            src_coord                    = src_coord_begin + scalar_id;
            r.s1(Number<2>{})            = load_data<float, float>(p_src, src_coord.GetOffset());

            scalar_id(vector_access_dim) = 3;
            src_coord                    = src_coord_begin + scalar_id;
            r.s1(Number<3>{})            = load_data<float, float>(p_src, src_coord.GetOffset());
    ...
    struct vector_data_store<float, 1, 4>
    {
        ...
        run(float* p_dst, const float_vec4_t src_data, const DstCoord dst_coord_begin)
        {
            ...
            store_data<float, float>(src_data.s1[Number<0>{}], p_dst, dst_coord.GetOffset());

            scalar_id(vector_access_dim) = 1;
            dst_coord                    = dst_coord_begin + scalar_id;
            store_data<float, float>(src_data.s1[Number<1>{}], p_dst, dst_coord.GetOffset());

            scalar_id(vector_access_dim) = 2;
            dst_coord                    = dst_coord_begin + scalar_id;
            store_data<float, float>(src_data.s1[Number<2>{}], p_dst, dst_coord.GetOffset());

            scalar_id(vector_access_dim) = 3;
            dst_coord                    = dst_coord_begin + scalar_id;
            store_data<float, float>(src_data.s1[Number<3>{}], p_dst, dst_coord.GetOffset());
        }
    };
    ...
    struct vector_data_store<float, 2, 2>
    {
        ...
        run(float* p_dst, const float_vec2_t src_data, const DstCoord dst_coord_begin)
        {
            ...
            scalar_id(vector_access_dim) = 0;
            auto dst_coord               = dst_coord_begin + scalar_id;
            store_data<float, float2_t>(src_data.s2, p_dst, dst_coord.GetOffset());
        }
    };
    ...
    struct vector_data_store<float, 4, 4>
    {
        ...
        run(float* p_dst, const float_vec4_t src_data, const DstCoord dst_coord_begin)
        {
            ...
            scalar_id(vector_access_dim) = 0;
            auto dst_coord               = dst_coord_begin + scalar_id;
            store_data<float, float4_t>(src_data.s4, p_dst, dst_coord.GetOffset());
        }
    };
